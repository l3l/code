...
class RB:
    ...
    def elabo_search(self, node, key):
        arr = [0, 0, 'NIL']
        sentinel = 0
        stack = stk()
        while not stack.is_empty() or node.val != None:
            if node.val != None:
                stack.push(node)
                node = node.left
            else:
                node = stack.pop()
                if key == arr[1] and sentinel:
                    arr[2] = node.val
                    arr[1] = arr[1]
                    if arr[0] == 0:
                        arr[0] = 'NIL'
                        ...
                        arr[0] = arr[0]
                    string = str(arr[0]) + ' ' + str(arr[1]) + ' ' + str(arr[2]) + ' '
                    return string
                arr[0] = arr[1]
                arr[1] = node.val
...
for line in lines:
    ...
    if int(line) == 0:
        break
f.close()
f1 = open('./prjtest/search01.txt', 'r')
f2 = open('./prjtest/output.txt', 'w')
lines = f1.readlines()
for line in lines:
    line = int(line)
    if line == 0:
        break
    data = rb.elabo_search(rb.root, line) + '\n'
    f2.write(data)
f2.close()
f1.close()
